...
> 위 표에서와 같이 세그먼트 레지스터에 담기는 값은 GDT 시작 주소로부터 Offset이 된다.<br>그런데, 글로벌 디스크립터의 크기는 8Byte라서 세그먼트 레지스터의 값은 0x08,0x10,0x18... 과 같은 값이 되고, 글로벌 디스크립터를 선택한다는 의미에서 셀렉터( *Selecter* ) 라는 이름으로 명명되었다.
>
