...
SECTION .text   ; text 섹션(세그먼트)을 정의한다.

jmp 0x07C0:START    ; CS 세그먼트 레지스터에 0x07C0을 복사하면서, START 레이블로 이동한다.

START:                          
    mov ax, 0x07C0              ; 부트로더의 시작 어드레스(0x7C00)를 세그먼트 레지스터 값으로 변환한다.
    mov ds, ax                  ; DS 세그먼트 레지스터에 설정
    mov ds, axmov as, 0xB800    ; 비디오 메모리의 시작 어드레스(0xB800)를 세그먼트 레지스터 값으로 변환한다.
    ...
    .SCREENCLEARLOOP:                   ; 화면을 지우는 루프
        ...
        cmp si, 80 * 25 * 2             ; 화면의 전체 크기는 80문자 * 25 라인이다.
                                        ; 출력한 문자의 수를 의미하는 SI 레지스터와 비교한다.
        jl .SCREENCLEARLOOPS            ; SI 레지스터가 80 * 25 * 2 보다 작다면 아직 지우지 못한
                                        ; 영역이 있으므로 .SCREENCLEARLOOP 레이블로 이동한다.


        mov si, 0                       ; SI 레지스터(문자열 원본 인덱스 레지스터)를 초기화한다.
        mov di, 0                       ; DI 레지스터(문자열 대상 인덱스 레지스터)를 초기화한다.

    .MESSAGELOOP:                       ; 메시지를 출력하는 루프
        mov cl, byte [ si + MESSAGE1 ]  ; MESSAGE1의 어드레스에서 SI 레지스터 값 만큼
                                        ; 더한 위치의 문자를 CL 레지스터에 복사
                                        ; CL 레지스터는 CX 레지스터의 하위 1바이트를 의미한다.
                                        ; 문자열은 1바이트면 충분하므로 CX 레지스터의 하위 1바이트만 사용한다.




        cmp cl, 0                       ; 복사된 문자와 0을 비교한다.
        je .MESSAGEEND                  ; 복사한 문자의 값이 0이면 문자열이 종료되었음을
                                        ; 의미하므로 .MESSAGEEND로 이동하여 문자 출력을 종료한다.

        mov byte [ es: di ], cl         ; 0이 아니라면 비디오 메모리 어드레스 0xB800 : di 에 문자를 출력한다.
    ...
    MESSAGE1:   db 'CYNOS64 OS Boot Loader Start!!', 0  ; 출력할 메시지 정의
                                                        ; 마지막은 0으로 설정하여 .MESSAGELOOP에서
                                                        ; 문자열이 종료 되었음을 알 수 있도록 한다.
